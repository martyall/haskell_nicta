...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P p) = P $ \s -> case p s of
  ErrorResult e -> ErrorResult e
...
list ::
  Parser a
  -> Parser (List a)
list p = list1 p ||| Course.Monad.return Nil
...
list1 ::
  Parser a
  -> Parser (List a)
list1 p = p Course.Monad.>>= \s -> (list p) Course.Monad.>>= \s' ->
  Course.Monad.return (s :. s')
...
digit1 :: Parser (Optional Int)
digit1 = read <$> (digit Course.Monad.>>= \d -> (list digit) Course.Monad.>>= \ds -> Course.Monad.return (d :. ds))
...
fromOptional :: Parser (Optional a) -> Parser a
fromOptional p = P $ \s -> let x = parse p s
                         in case x of
                           ErrorResult e -> ErrorResult e
                           Result s' (Full a) -> Result s' a
natural ::
  Parser Int
natural = fromOptional digit1
...
spaces1 ::
  Parser Chars
spaces1 = space Course.Monad.>>= \x -> (list space) Course.Monad.>>= \xs -> Course.Monad.return (x:.xs)
...
alpha ::
  Parser Char
alpha = satisfy Data.Char.isUpper
...
firstNameParser ::
  Parser Chars
firstNameParser = upper Course.Monad.>>= \x -> (list lower) Course.Monad.>>= \xs -> Course.Monad.return (x:.xs)
...
phoneParser ::
  Parser Chars
phoneParser = digit Course.Monad.>>= \d -> phoneBodyParser Course.Monad.>>=
  \b -> (is '#') >> Course.Monad.return (d:.b)
...
personParser ::
  Parser Person
personParser = ageParser >>= \a -> spaces1 >> firstNameParser >>= \fn -> spaces1 >> surnameParser >>= \sn -> spaces1 >> smokerParser >>= \sm -> spaces1 >> phoneParser >>= \pn -> return $ Person a fn sn sm pn
...
instance Apply Parser where
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) f p = P $ \s -> case ((parse f) s) of
    (ErrorResult e) -> ErrorResult e
    (Result s' phi) ->  parse (phi <$> p) s'
...
instance Course.Monad.Monad Parser where
  return =  pure
  (>>=) = flbindParser
